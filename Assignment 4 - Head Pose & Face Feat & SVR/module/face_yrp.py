...
class FaceYRP:
    ...
    def get_orientation(self, img: numpy.ndarray) -> (numpy.ndarray, numpy.ndarray, tuple, tuple):
        ...
        image_points = FaceYRP.get_image_points(landmarks)
        ...
        img_prj_points, img_jac = cv2.projectPoints(axis, rotation_vector, translation_vector,
                                                    camera_matrix, self.dist_coeffs)

        obj_prj_points, obj_jac = cv2.projectPoints(object_points, rotation_vector, translation_vector,
                                                    camera_matrix, self.dist_coeffs)
        ...
        nose_coord = landmarks[31]

        # print(self.pitch, self.roll, self.yaw)
        self.result = True
        return img_prj_points, obj_prj_points, \
               (int(self.roll), int(self.pitch), int(self.yaw)), \
               nose_coord, image_points
